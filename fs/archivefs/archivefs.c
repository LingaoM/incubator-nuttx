...
static off_t archivefs_skip(FAR struct archivefs_priv_s *priv, off_t amount)
{
  ...
  if (priv->seekbuf == NULL)
    {
      priv->seekbuf = kmm_malloc(CONFIG_FS_ARCHIVEFS_BUFFER_SIZE);
      if (priv->seekbuf == NULL)
        {
          return -ENOMEM;
        }
                  ...
                  FAR struct archivefs_priv_s **priv)
{
  ...
  newp = kmm_zalloc(sizeof(struct archivefs_priv_s));
  if (newp == NULL)
    {
      return ret;
    }
                             ...
                             FAR struct fs_dirent_s **dir)
{
  ...
  adir = kmm_zalloc(sizeof(struct archivefs_dir_s));
  if (adir == NULL)
    {
      return -ENOMEM;
    }
