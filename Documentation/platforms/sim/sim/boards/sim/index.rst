...
The simulator connects to the BabbleSim PHY only when explicitly requested at
runtime:

.. code:: console

   $ ./nuttx --sim-bsim-enabled \
       --sim-bsim-sid=default \
       --sim-bsim-pid=2G4 \
       --sim-bsim-dev=0
...
``CONFIG_SIM_BSIM_PHY_ID``, and ``CONFIG_SIM_BSIM_DEVICE_NBR``.  If
``--sim-bsim-enabled`` is omitted, the simulator keeps using the normal
wall-time source even when BSIM support was compiled in.
