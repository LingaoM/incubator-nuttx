 ...
 *
 * If there ever is a STM32L5 with D-cache, the buffer size
...
static int  stm32l5serial_interrupt(int irq, void *context,
                                    void *arg);
...
static void stm32l5serial_pmnotify(struct pm_callback_s *cb, int domain,
                                   enum pm_state_e pmstate);
static int  stm32l5serial_pmprepare(struct pm_callback_s *cb, int domain,
                                    enum pm_state_e pmstate);
#endif
...
{
  .setup          = stm32l5serial_setup,
  .shutdown       = stm32l5serial_shutdown,
  .attach         = stm32l5serial_attach,
  .detach         = stm32l5serial_detach,
  .ioctl          = stm32l5serial_ioctl,
...
{
  .setup          = stm32l5serial_dmasetup,
  .shutdown       = stm32l5serial_dmashutdown,
  .attach         = stm32l5serial_attach,
  .detach         = stm32l5serial_detach,
  .ioctl          = stm32l5serial_ioctl,
...
{
  .pm_cb.notify  = stm32l5serial_pmnotify,
  .pm_cb.prepare = stm32l5serial_pmprepare,
  ...
  if (suspend)
    {
      ...
      stm32l5serial_disableusartint(priv, &priv->suspended_ie);
  ...
  else
    {
      ...
      if (priv->dev.ops == &g_uart_dma_ops && priv->rxdmasusp)
        {
          ...
          if (priv->iflow)
...
#endif
            {
              ...
              stm32l5serial_dmareenable(priv);
      ...
      stm32l5serial_restoreusartint(priv, priv->suspended_ie);
  ...
  if (dmarestored)
    {
      ...
      if (priv->rxdma != NULL)
        {
          stm32l5serial_dmarxcallback(priv->rxdma, 0, priv);
        }
  ...
  for (n = 0; n < STM32L5_NLPUART + STM32L5_NUSART + STM32L5_NUART; n++)
    {
      ...
      stm32l5serial_setsuspend(&priv->dev, suspend);
    }
}
#endif
...
static int stm32l5serial_setup(struct uart_dev_s *dev)
{
  ...
  stm32l5serial_setapbclock(dev, true);
...
static int stm32l5serial_dmasetup(struct uart_dev_s *dev)
{
  ...
  if (!dev->isconsole)
    {
      result = stm32l5serial_setup(dev);
      if (result != OK)
        {
          return result;
        }
...
static void stm32l5serial_shutdown(struct uart_dev_s *dev)
{
  ...
  stm32l5serial_disableusartint(priv, NULL);

  /* Disable USART APB1/2 clock */

  stm32l5serial_setapbclock(dev, false);
...
static int stm32l5serial_attach(struct uart_dev_s *dev)
{
  ...
  ret = irq_attach(priv->irq, stm32l5serial_interrupt, priv);
      ...
      if (priv->rxdmasusp)
        {
          ...
          if (stm32l5serial_dmarxavailable(&priv->dev))
            {
...
static bool stm32l5serial_dmarxavailable(struct uart_dev_s *dev)
{
  ...
  return (stm32l5serial_dmanextrx(priv) != priv->rxdmanext);
}
#endif
...
#  endif
  ...
  else
    {
      ...
      stm32l5serial_restoreusartint(priv, priv->ie & ~USART_CR1_TXEIE);
    }
  ...
  switch (pmstate)
    {
      case PM_NORMAL:
        {
          stm32l5serial_pm_setsuspend(false);
        }
        break;
      ...
      case PM_STANDBY:
        {
          ...
          stm32l5serial_pm_setsuspend(true);
        }
        break;
  ...
  for (i = 0; i < STM32L5_NLPUART + STM32L5_NUSART + STM32L5_NUART; i++)
    {
      if (g_uart_devs[i])
        {
          stm32l5serial_disableusartint(g_uart_devs[i], NULL);
        }
    }
...
#if CONSOLE_UART > 0
  stm32l5serial_setup(&g_uart_devs[CONSOLE_UART - 1]->dev);
#endif
#endif /* HAVE UART */
}
...
void arm_serialinit(void)
{
  ...
  stm32l5serial_dmasetup(&g_uart_devs[CONSOLE_UART - 1]->dev);
#endif
...
void stm32l5_serial_dma_poll(void)
{
  ...
  if (g_lpuart1priv.rxdma != NULL)
    {
      stm32l5serial_dmarxcallback(g_lpuart1priv.rxdma, 0, &g_lpuart1priv);
    }
#endif
  ...
  if (g_usart1priv.rxdma != NULL)
    {
      stm32l5serial_dmarxcallback(g_usart1priv.rxdma, 0, &g_usart1priv);
    }
#endif
  ...
  if (g_usart2priv.rxdma != NULL)
    {
      stm32l5serial_dmarxcallback(g_usart2priv.rxdma, 0, &g_usart2priv);
    }
#endif
  ...
  if (g_usart3priv.rxdma != NULL)
    {
      stm32l5serial_dmarxcallback(g_usart3priv.rxdma, 0, &g_usart3priv);
    }
#endif
  ...
  if (g_uart4priv.rxdma != NULL)
    {
      stm32l5serial_dmarxcallback(g_uart4priv.rxdma, 0, &g_uart4priv);
    }
#endif
  ...
  if (g_uart5priv.rxdma != NULL)
    {
      stm32l5serial_dmarxcallback(g_uart5priv.rxdma, 0, &g_uart5priv);
    }
#endif
