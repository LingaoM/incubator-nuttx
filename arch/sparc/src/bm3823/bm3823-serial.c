...
#    if defined(CONFIG_BM3823_UART2)
#      define TTYS1_DEV     g_uart2port     /* UART2 is ttyS1 */
#    elif defined(CONFIG_BM3823_UART3)
...
#    if defined(CONFIG_BM3823_UART3)
#      define TTYS1_DEV     g_uart3port     /* UART3 is ttyS1 */
#    else
#      undef  TTYS1_DEV                     /* No ttyS1 */
#    endif
...
#  if defined(CONFIG_BM3823_UART1)
#    define TTYS0_DEV       g_uart1port     /* UART1 is ttyS0 */
#    if defined(CONFIG_BM3823_UART2)
#      define TTYS1_DEV     g_uart2port     /* UART2 is ttyS1 */
#    elif defined(CONFIG_BM3823_UART3)
...
#      undef  TTYS1_DEV                     /* No ttyS1 */
#    endif
#  elif defined(CONFIG_BM3823_UART2)
...
#endif
/* This describes the state of the BM3823 UART1 port. */

#ifdef CONFIG_BM3823_UART1
static struct up_dev_s g_uart1priv =
{
  .uartbase  = BM3823_UART1_BASE,
  .baud      = CONFIG_UART1_BAUD,
  .irq       = BM3823_IRQ_UART_1_RX_TX,
...
/* This describes the state of the BM3823 UART2 port. */

#ifdef CONFIG_BM3823_UART2
static struct up_dev_s g_uart2priv =
{
  .uartbase  = BM3823_UART2_BASE,
...
/* This describes the state of the BM3823 UART3 port. */

#ifdef CONFIG_BM3823_UART3
static struct up_dev_s g_uart3priv =
{
  .uartbase  = BM3823_UART3_BASE,
  .baud      = CONFIG_UART3_BAUD,
  .irq       = BM3823_IRQ_UART_3_RX_TX,
...
static inline void up_setuartint(struct up_dev_s *priv)
{
  ...
  regval   = up_serialin(priv, BM3823_UART_CTRLREG_OFFSET);
  regval  &= ~MSK_UART_ALLINTS;
  regval  |= priv->im;
  up_serialout(priv, BM3823_UART_CTRLREG_OFFSET, regval);
}
...
static int up_setup(struct uart_dev_s *dev)
{
  ...
  bm3823_uartconfigure(priv->uartbase, priv->baud, priv->parity,
...
static void up_shutdown(struct uart_dev_s *dev)
{
  ...
  /* bm3823_uartreset(priv->uartbase); */
}
  ...
  for (passes = 0; passes < 256 && handled; passes++)
    {
      ...
      /* Get the masked UART status and clear the pending interrupts. */
       ...
       mis = up_serialin(priv, BM3823_UART_STATREG_OFFSET);
...
static int up_receive(struct uart_dev_s *dev, uint32_t *status)
{
  ...
  return  (int)(up_serialin(priv, BM3823_UART_RXREG_OFFSET) &
               UART_RXREG_MASK);
}
...
static bool up_rxavailable(struct uart_dev_s *dev)
{
  ...
  return (up_serialin(priv, BM3823_UART_STATREG_OFFSET) &
...
{
  struct up_dev_s *priv = (struct up_dev_s *)dev->priv;
  up_serialout(priv, BM3823_UART_TXREG_OFFSET, (uint32_t)ch);
}
